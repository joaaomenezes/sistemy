...
**Como validar antes de remover:** Buscar no GitHub/Netlify se existe rota publicada ou link externo apontando para `dashboard-prototipo.html`; perguntar se ainda serve como referencia de design.  
**Risco:** Baixo
...
**Arquivo:** `vercel.json`  
**Motivo:** O frontend esta documentado como Netlify. `vercel.json` redireciona para `landing.html`, mas nao e usado pelo Netlify.  
**Possivel uso indireto:** Pode existir deploy antigo/alternativo na Vercel.  
**Como validar antes de remover:** Confirmar que nao existe projeto ativo na Vercel para este repositorio.  
**Risco:** Baixo
...
**Arquivo:** `build.sh`  
**Motivo:** Gera `config.js` a partir de `API_URL`, mas o repo possui `config.js` versionado apontando para Railway. Nao foi encontrado `netlify.toml` no repo.  
**Possivel uso indireto:** Pode estar configurado manualmente como build command no Netlify.  
**Como validar antes de remover:** Conferir no painel Netlify se o build command usa `build.sh`. Se nao usar, remover.  
**Risco:** Medio
